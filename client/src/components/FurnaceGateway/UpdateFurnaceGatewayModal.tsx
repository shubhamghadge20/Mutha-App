...
import { updateFurnaceGatewayThunk } from "@/features/FurnaceGateway";
import { UpdateFurnaceGatewayInterface, FurnaceGateway } from "@/types";

interface UpdateFurnaceGatewayModalProps {
  open: boolean;
  onClose: () => void;
  gatewayData: FurnaceGateway | null;
}

const UpdateFurnaceGatewayModal = ({
  open,
  onClose,
  gatewayData,
}: UpdateFurnaceGatewayModalProps) => {
  const dispatch = useAppDispatch();
  const { loading } = useAppSelector((state) => state.furnaceGateway);

  const [formData, setFormData] = useState<UpdateFurnaceGatewayInterface>({
    id: "",
    furnaceId: "",
    gatewayMac: "",
  });

  const [errors, setErrors] = useState<Partial<UpdateFurnaceGatewayInterface>>(
    {}
  );
  ...
  useEffect(() => {
    if (gatewayData) {
      setFormData({
        id: gatewayData.id,
        furnaceId: gatewayData.furnaceId,
  ...
  const validateForm = () => {
    const newErrors: Partial<UpdateFurnaceGatewayInterface> = {};
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await dispatch(
      updateFurnaceGatewayThunk({ id: formData.id, data: formData })
    );
    if (updateFurnaceGatewayThunk.fulfilled.match(result)) {
      onClose();
    }
  };

  if (!open || !gatewayData) return null;
...
export default UpdateFurnaceGatewayModal;
